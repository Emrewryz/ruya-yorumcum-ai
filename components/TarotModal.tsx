    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          {/* ANA MODAL ALANI */}
          <div className="relative z-[105] w-full max-w-5xl px-4 flex flex-col items-center justify-center min-h-[600px]">
            ...
            {step === 'CHOICE' && (
              <motion.h2 
                ...
                className="font-serif text-3xl text-white mb-12 tracking-widest text-center"
              >
                KADERİNİ SEÇ
                <span className="block text-xs text-[#8b5cf6] mt-2 font-sans tracking-[0.5em]">ENERJİ HİZALANDI</span>
              </motion.h2>
            )}

            {/* KARTLAR ALANI */}
            <div className="flex items-center justify-center gap-4 md:gap-12 h-[400px] perspective-1000">
              {[1, 2, 3].map((cardId) => {
                ...
                return (
                  <motion.div
                    ...
                    animate={
                      isSelected 
                      ? { 
                          x: 0, y: 0, scale: 1.5, rotateY: 180, zIndex: 50,
                    ...
                    className={`relative w-48 h-80 rounded-2xl cursor-pointer preserve-3d transition-shadow duration-500 group ${step === 'CHOICE' ? 'shadow-[0_0_20px_rgba(139,92,246,0.3)]' : ''}`}
                    style={{ transformStyle: "preserve-3d" }}
                  >
                    {/* --- KARTIN ARKASI (Mistik Desen) --- */}
                    <div className="absolute inset-0 w-full h-full backface-hidden rounded-2xl overflow-hidden bg-[#0f172a] border border-[#fbbf24]/30">
                       {/* Gold Foil Gradient */}
                       ...
                       <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <div className="w-20 h-20 rounded-full border-2 border-[#fbbf24]/50 flex items-center justify-center mb-2 group-hover:border-[#fbbf24] group-hover:shadow-[0_0_20px_#fbbf24] transition-all">
                             <Moon className="w-10 h-10 text-[#fbbf24]" />
                          </div>
                          <div className="h-[1px] w-12 bg-[#fbbf24]/50"></div>
                       </div>
                       
                       {/* Kenar Dumanı (Glow) */}
                       <div className="absolute inset-0 shadow-[inset_0_0_30px_rgba(139,92,246,0.2)] group-hover:shadow-[inset_0_0_40px_rgba(251,191,36,0.2)] transition-shadow duration-500"></div>
                    </div>

                    {/* --- KARTIN ÖNÜ (Reveal Sonrası) --- */}
                    <div 
                      className="absolute inset-0 w-full h-full backface-hidden rounded-2xl overflow-hidden bg-black border-2 border-white/10"
                       ...
                       {revealedCard && (
                         <>
                           ...
                           <div className="absolute bottom-4 left-0 right-0 text-center">
                              <h3 className="font-serif text-xl font-bold text-white tracking-widest uppercase">{revealedCard.name}</h3>
                           </div>
            ...
            {step === 'INTERPRETATION' && revealedCard && (
              <motion.div 
                ...
                className="mt-12 w-full max-w-2xl bg-black/40 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl relative overflow-hidden"
              >
                 {/* Başlık */}
                 <div className="flex items-center gap-3 mb-4 text-[#fbbf24]">
                    <Sparkles className="w-5 h-5" />
                    <h3 className="font-serif text-sm tracking-widest uppercase">Rüya ve Kartın Sentezi</h3>
                 </div>

                 {/* Daktilo Metni */}
                 <p className="text-lg text-gray-300 leading-relaxed font-light min-h-[100px]">
                    {renderStyledText(streamedText)}
                    <span className="inline-block w-1.5 h-5 bg-[#fbbf24] ml-1 animate-pulse align-middle"></span>
                 </p>
