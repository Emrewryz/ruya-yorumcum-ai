...
import { BrainCircuit, Layers, Hash, Star } from "lucide-react"; 
...
const SERVICES = [
  {
    ...
    theme: { 
      ...
      gradient: 'from-amber-500/10 dark:from-amber-500/10', 
...
export default function BlogSidebarCTA() {
  ...
  return (
    <div className={`bg-gradient-to-br ${activeService.theme.gradient} to-[var(--bg-card)] border ${activeService.theme.border} rounded-[2rem] p-6 relative overflow-hidden shadow-2xl shadow-slate-200/50 dark:shadow-black/50 min-h-[260px] transition-colors duration-1000`}>
        <div className="absolute -top-4 -right-4 p-4 opacity-10 transition-opacity pointer-events-none">
        ...
        <AnimatePresence mode="wait">
            ...
            >
                ...
                <p className="text-xs text-[var(--text-muted)] font-light leading-relaxed mb-6 min-h-[60px]">
                  {activeService.desc}
                </p>
                <Link href={activeService.link} className={`w-full py-3 ${activeService.theme.bg} ${activeService.theme.hover} text-white dark:text-[#0B0F19] rounded-xl text-sm font-bold shadow-lg shadow-${activeService.theme.border} transition-colors flex items-center justify-center gap-2 mt-auto active:scale-95`}>
        ...
        <div className="absolute bottom-3 left-0 w-full flex justify-center gap-1.5 z-20">
            ...
            <div 
                key={idx} 
                className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${idx === currentIndex ? 'bg-slate-800 dark:bg-white scale-125' : 'bg-slate-300 dark:bg-white/20'}`}
            />
            ))}
        </div>
    </div>
