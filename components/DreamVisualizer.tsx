...
function ImageCard({ url, showShareHint = false }: { url: string; showShareHint?: boolean }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm animate-in fade-in zoom-in-95 duration-500">
...
export default function DreamVisualizer({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Mevcut görsel */}
      {hasExisting && <ImageCard url={existingImageUrl!} />}

      {/* Yeni üretilen görsel */}
      {hasNew && <ImageCard url={newImageUrl!} showShareHint />}

      {/* ── Boş Durum: Aydınlık Premium Banner ── */}
      {phase === "idle" && !hasExisting && !hasNew && (
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-b from-zinc-50 to-white p-8 shadow-sm">
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1.5 text-sm text-zinc-500 leading-relaxed max-w-xs mx-auto">
                Bilinçaltınızdaki sembolleri yapay zeka ile sinematik bir sanat eserine dönüştürün.
              </p>
            </div>
            <button
              onClick={handleGenerate}
              className="flex items-center gap-2 rounded-xl border border-zinc-200 bg-white px-5 py-2.5 text-sm font-semibold text-zinc-900 shadow-sm hover:border-zinc-300 hover:bg-zinc-50 transition-colors active:scale-[0.98]"
            >
              <Sparkles className="h-4 w-4 text-zinc-500" strokeWidth={1.5} />
              Görsele Dönüştür
              <span className="rounded-lg border border-zinc-100 bg-zinc-50 px-2 py-0.5 text-xs font-medium text-zinc-400">
                4 Kredi
              </span>
            </button>
          </div>
      ...
      {/* ── Generating: Aydınlık Loading ── */}
      {phase === "generating" && (
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-b from-zinc-50 to-white p-8 shadow-sm">
          <div className="flex items-center gap-3 mb-5">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-zinc-200 bg-white shadow-sm">
              <Sparkles className="h-5 w-5 text-zinc-400 animate-pulse" strokeWidth={1.5} />
            </div>
            <div>
              <p className="text-sm font-semibold text-zinc-800">
                {LOADING_STEPS[stepIndex]}
              </p>
              <p className="mt-0.5 text-xs text-zinc-400">~20-30 saniye</p>
            </div>
          </div>
          ...
          <div className="h-1 w-full rounded-full bg-zinc-100 overflow-hidden">
            <div
              className="h-full rounded-full bg-zinc-300 transition-all duration-[4500ms] ease-linear"
              style={{ width: `${((stepIndex + 1) / LOADING_STEPS.length) * 100}%` }}
            />
          </div>

          {/* Shimmer placeholder */}
          <div className="mt-4 aspect-square w-full max-w-xs mx-auto rounded-xl bg-zinc-100 overflow-hidden relative">
