...
import { 
  ...
  Moon, GitGraph, LayoutGrid // Yeni ikonlar ekledim
} from "lucide-react";
...
// Sol menüdeki kategorik rüyalar için tip
...
// Alt kısımdaki benzer rüyalar için tip
...
// 2. SOL MENÜ: Kategori Bazlı Rüyalar (RPC Kullanarak)
// 2. SOL MENÜ: Kategori Bazlı Rüyalar
const getCategoryDreams = async (category: string | null, currentId: string): Promise<CategoryDream[]> => {
  ...
  // Gelen veriyi (data) CategoryDream dizisi olarak zorluyoruz
  return (data as CategoryDream[]) || [];
};

// 3. ALT KISIM: Keyword Bazlı Rüyalar
const getRelatedDreams = async (keywords: string[] | null, currentId: string): Promise<RelatedDream[]> => {
  ...
  // Gelen veriyi (data) RelatedDream dizisi olarak zorluyoruz
...
export default async function Page({ params }: { params: { slug: string } }) {
  // 1. Önce Ana Veriyi Çek
  const item = await getDreamData(params.slug);
  const supabase = createClient();

  if (!item) return <div className="text-white text-center py-20">İçerik bulunamadı.</div>;

  // 2. Ana veri geldikten sonra, elimizdeki Kategori ve Keyword'e göre yan verileri çek
  // Promise.all ile paralel çekiyoruz ki hız düşmesin.
  const [categoryDreams, relatedDreams] = await Promise.all([
    ...
    supabase.rpc('increment_search_count', { row_id: item.id }) // Sayaç artır
  ]);
    ...
    <div className="min-h-screen bg-[#020617] text-white font-sans pb-20 pt-28">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <article className="col-span-1 lg:col-span-9">
           ...
           {/* HERO SECTION (Değişmedi) */}
           <header className="relative p-8 md:p-12 rounded-[2rem] bg-[#0a0a0a] border border-white/5 overflow-hidden mb-12 shadow-2xl">
                              ...
                              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pl-4">
                                 ...
                                 {scene.slug && validSlugs.includes(scene.slug) && (
                                    <Link 
                                      ...
                                      className="shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-lg border border-white/10 bg-white/5 hover:bg-[#fbbf24] hover:border-[#fbbf24] hover:text-black text-xs font-bold text-gray-400 transition-all uppercase tracking-wider whitespace-nowrap"
                                    >
           ...
           {/* --- ADIM 3: YENİ ALT BÖLÜM (KEYWORD BASED) --- */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {relatedDreams.map((related) => (
                        <Link 
                            ...
                            className="group p-5 bg-[#0f172a] border border-white/5 rounded-2xl hover:border-blue-500/30 hover:bg-[#0f172a]/80 transition-all"
                        >
                            <div className="flex items-start justify-between mb-3">
                                <h4 className="text-lg font-bold text-gray-200 group-hover:text-blue-400 transition-colors">
