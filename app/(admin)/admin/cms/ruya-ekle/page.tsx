...
import { Loader2, CheckCircle, AlertCircle, Upload, Eye, Save } from "lucide-react";
...
// ─── Şu anki zaman — datetime-local formatında ───────────────────────────────
...
export default function RuyaEklePage() {
  ...
  // JSON değişince önizleme güncelle
  ...
  function handleSubmit(isPublished: boolean) {
    setResult(null);
    if (!jsonText.trim()) {
      setResult({ ok: false, msg: "JSON alanı boş." });
      return;
    }
    ...
    if (!term?.trim()) {
      setResult({ ok: false, msg: 'JSON içinde "term" alanı bulunamadı.' });
      return;
    }
    if (!description?.trim()) {
      setResult({ ok: false, msg: 'JSON içinde "description" alanı bulunamadı.' });
      return;
    }
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
          ...
          <p className="mt-2 text-xs text-zinc-600">
            {new Date(publishedAt) > new Date()
              ? "⏰ Planlandı — ileri tarih, Google'dan gizlenir."
              : "✓ Anında yayınlanır."}
          </p>
        </div>
            ...
            <div className="mt-3 rounded-xl border border-zinc-700/50 bg-zinc-800/60 px-4 py-3">
              ...
              <div className="space-y-1 text-xs">
                <div className="flex gap-2">
                  <span className="text-zinc-600 w-24 shrink-0">Term:</span>
                  <span className="text-white font-medium">{preview.term ?? "—"}</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-zinc-600 w-24 shrink-0">Slug:</span>
                ...
                <div className="flex gap-2">
                  <span className="text-zinc-600 w-24 shrink-0">Açıklama:</span>
                  <span className="text-zinc-300 line-clamp-1">{preview.description ?? "—"}</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-zinc-600 w-24 shrink-0">Etiketler:</span>
                  <span className="text-zinc-300">
                    {Array.isArray(preview.tags) ? preview.tags.join(", ") : "—"}
                  </span>
                </div>
                <div className="flex gap-2">
                  <span className="text-zinc-600 w-24 shrink-0">İçerik tipi:</span>
        ...
        {result && (
          <div className={`flex items-start gap-2.5 rounded-xl px-4 py-3 text-sm ${
            ...
            {result.ok
              ? <CheckCircle className="h-4 w-4 mt-0.5 shrink-0" strokeWidth={1.5} />
