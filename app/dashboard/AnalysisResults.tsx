...
import { Brain, Moon, Lock, Check, Palette, ImageIcon, Download, Heart, Star, ChevronRight, Layers, MessageCircle, Sparkles } from "lucide-react";
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="pb-32 space-y-12">
      ...
      <div className="text-center mb-16">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#fbbf24]/10 border border-[#fbbf24]/20 text-[#fbbf24] text-xs font-bold uppercase tracking-widest mb-6"><Check className="w-4 h-4" /> Analiz Tamamlandı</div>
        <h2 className="font-serif text-3xl md:text-5xl font-bold text-white mb-8 leading-tight">{result.title || "Rüyanın Gizli Mesajı"}</h2>
      ...
      <div className="grid md:grid-cols-2 gap-8">
         <div className="relative p-8 rounded-[2.5rem] bg-[#0f172a] border border-blue-500/10 hover:border-blue-500/30 transition-colors shadow-2xl overflow-hidden group">
            <h3 className="font-serif text-2xl text-blue-400 mb-6 flex items-center gap-3"><Brain className="w-6 h-6" /> Bilinçaltı</h3>
         ...
         <div className="relative p-8 rounded-[2.5rem] bg-[#022c22] border border-emerald-500/10 hover:border-emerald-500/30 transition-colors shadow-2xl overflow-hidden group">
            <h3 className="font-serif text-2xl text-emerald-400 mb-6 flex items-center gap-3"><Moon className="w-6 h-6" /> Manevi Mesaj</h3>
      ...
      {/* 3. Görsel, Mood, Numeroloji ve Tarot Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         ...
         <div className="relative col-span-1 md:col-span-2 rounded-[2rem] overflow-hidden bg-[#080808] border border-white/10 group shadow-2xl min-h-[450px]">
            <div className="grid md:grid-cols-2 h-full relative z-10">
               <div className="p-10 flex flex-col justify-center">
                  <h3 className="font-serif text-3xl text-white mb-6">Rüyanın <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-fuchsia-400">Sanat Hali</span></h3>
                  <p className="text-gray-400 mb-10 text-sm">Bilinçaltındaki görüntüleri dijital sanata dönüştür.</p>
                  {hasAccess('pro') ? (
                     <button onClick={() => router.push(`/dashboard/gorsel-olustur/${currentDreamId}`)} className="self-start px-8 py-4 rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white font-bold text-sm flex items-center gap-3">
               ...
               <div className="relative h-[300px] md:h-full bg-[#050505] border-l border-white/10">
                  {generatedImage ? (
                     <>
         ...
         {/* Mood */}
         <div onClick={() => hasAccess('pro') && router.push('/dashboard/duygu-durumu')} className="relative group rounded-[2rem] bg-[#080808] border border-white/10 p-8 overflow-hidden hover:border-rose-500/30 transition-all cursor-pointer min-h-[320px]">
            ...
            <h3 className="font-serif text-4xl text-rose-400 mb-2">{result.mood}</h3>
         ...
         {/* Numeroloji */}
         <div className="relative group rounded-[2rem] bg-[#080808] border border-white/10 p-8 overflow-hidden hover:border-sky-500/30 transition-all min-h-[320px]">
            ...
            <div className="flex flex-wrap gap-3 mb-6">
               {result.lucky_numbers?.map((num: any, i: number) => (
                  <div key={i} className="w-12 h-14 rounded-xl bg-[#0f172a] border border-sky-500/20 flex items-center justify-center font-mono text-xl text-sky-400">{num}</div>
               ))}
            </div>
         ...
         {/* Tarot */}
         <div className="relative group rounded-[2rem] bg-[#080808] border border-white/10 p-8 overflow-hidden hover:border-amber-500/30 transition-all min-h-[320px]">
            ...
            <div className="flex items-center gap-4 bg-amber-900/10 p-4 rounded-2xl mb-6"><div className="w-12 h-16 bg-gradient-to-br from-amber-700 to-amber-900 rounded border border-amber-500/40"></div><div className="text-amber-400 font-serif text-lg">Gizli Kart</div></div>
         ...
         {/* Chat (Elite) */}
         <div className="relative col-span-1 md:col-span-2 rounded-[2rem] bg-[#0c0a09] border border-white/10 p-8 flex items-center justify-between group hover:border-orange-500/30 transition-all">
            {!hasAccess('elite') && <LockedFeature title="Kahin Sohbet" />}
            <div className="flex items-center gap-4"><div className="p-3 rounded-2xl bg-orange-500/20"><MessageCircle className="w-6 h-6 text-orange-400" /></div><div><h3 className="font-serif text-xl text-white">Rüya Kahini ile Konuş</h3><p className="text-gray-400 text-xs">Sorular sorarak analizi derinleştir.</p></div></div>
            <button onClick={() => hasAccess('elite') && router.push(`/dashboard/sohbet/${currentDreamId}`)} className="px-6 py-3 rounded-full bg-white text-black font-bold text-xs uppercase">Sohbeti Başlat</button>
         </div>
