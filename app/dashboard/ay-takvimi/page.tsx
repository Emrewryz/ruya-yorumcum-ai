...
export default function MoonCalendarPage() {
  ...
  return (
    ...
    <div className="min-h-[100dvh] bg-[#020617] text-white font-sans relative overflow-x-hidden flex flex-col items-center pb-32">
      ...
      <main className="w-full max-w-4xl px-4 md:px-6 relative z-10 text-center">
         ...
         <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <div className="mb-4 inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-widest backdrop-blur-md">
               <Calendar className="w-3 h-3" /> {today}
            </div>

            <div className="relative w-48 h-48 md:w-80 md:h-80 mx-auto my-8 md:my-10 flex items-center justify-center">
               <div className="absolute inset-0 bg-[#fbbf24] blur-[60px] md:blur-[80px] opacity-20 animate-pulse-slow"></div>
               <div className="text-[120px] md:text-[200px] leading-none select-none filter drop-shadow-[0_0_30px_rgba(255,255,255,0.3)] transform hover:scale-105 transition-transform duration-700">
               {currentMoon.icon}
               </div>
               {/* Dekoratif Yörüngeler */}
               <div className="absolute inset-0 border border-white/5 rounded-full scale-150 animate-spin-slow-reverse pointer-events-none"></div>
               <div className="absolute inset-0 border border-white/5 rounded-full scale-125 border-dashed animate-spin-slow pointer-events-none"></div>
            </div>

            <h2 className="text-3xl md:text-6xl font-serif font-bold text-white mb-4 md:mb-6 bg-clip-text text-transparent bg-gradient-to-b from-white to-gray-400">
               {currentMoon.phase}
            </h2>
            
            <p className="text-base md:text-xl text-[#fbbf24] font-medium mb-12 max-w-2xl mx-auto italic leading-relaxed px-4">
         ...
         <motion.div 
            ...
            className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 text-left mb-16"
         >
            {/* Rüyalara Etkisi */}
            <div className="p-6 md:p-8 rounded-3xl bg-[#0f172a] border border-[#8b5cf6]/30 relative overflow-hidden group">
               <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  <CloudFog className="w-20 h-20 md:w-24 md:h-24 text-[#8b5cf6]" />
               </div>
            ...
            {/* Enerji Durumu */}
            <div className="p-6 md:p-8 rounded-3xl bg-[#0f172a] border border-[#fbbf24]/30 relative overflow-hidden group">
               <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                  <Star className="w-20 h-20 md:w-24 md:h-24 text-[#fbbf24]" />
               </div>
               <div className="flex items-center gap-3 mb-4">
                ...
                {nextDays.map((day, i) => (
                    <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors">
                        ...
                        <div className="flex-1 text-left">
                           <div className="text-[#fbbf24] font-bold text-xs md:text-sm">{day.date}</div>
                           <div className="text-white font-serif text-base md:text-lg">{day.phase}</div>
                        </div>
