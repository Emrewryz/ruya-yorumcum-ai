...
export default function PricingPage() {
  ...
  const handlePurchase = async (planName: string) => {
    ...
    // PROFESYONEL BİLGİLENDİRME KARTI (MOBİL UYUMLU)
    toast.custom((t) => (
        ...
        <div className="p-5 md:p-6 relative z-10">
          ...
          <div className="flex items-start justify-between mb-5">
            <div className="flex items-center gap-3">
               <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-amber-500/10 flex items-center justify-center border border-amber-500/20 shrink-0">
                  <ShieldCheck className="w-4 h-4 md:w-5 md:h-5 text-amber-400" />
               </div>
               <div>
                  <h3 className="text-white font-bold text-sm md:text-base leading-none">Güvenli Ödeme Adımı</h3>
                  <p className="text-gray-400 text-[10px] md:text-xs mt-1">Otomatik aktivasyon için önemlidir.</p>
               </div>
            </div>
            <button 
          ...
          >
              <div className="flex items-center justify-between mb-1">
                 <span className="text-[9px] md:text-[10px] uppercase tracking-wider text-gray-500 font-bold group-hover:text-amber-500 transition-colors">
                   KULLANMANIZ GEREKEN E-POSTA
                 </span>
                 <span className="text-[9px] md:text-[10px] bg-white/10 text-gray-300 px-2 py-0.5 rounded flex items-center gap-1 group-hover:bg-amber-500 group-hover:text-black transition-colors">
                    <Copy className="w-3 h-3" /> KOPYALA
                 </span>
              </div>
              <div className="font-mono text-xs md:text-sm text-white truncate group-hover:text-amber-100">
                {user.email}
              </div>
          </div>
          ...
          <div className="bg-blue-500/5 border border-blue-500/10 rounded-lg p-3 mb-5 flex gap-3">
             ...
             <div className="text-[10px] md:text-[11px] leading-relaxed text-gray-300">
          ...
          <button 
            ...
            className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white py-3 md:py-3.5 rounded-xl font-bold text-xs md:text-sm shadow-lg shadow-green-900/20 flex items-center justify-center gap-2 transition-all active:scale-95"
          >
              ANLADIM, ÖDEMEYE GİT <ShieldCheck className="w-3 h-3 md:w-4 md:h-4" />
          </button>
        </div>
      </div>
    ), { 
  ...
  const plans: Plan[] = [
    ...
    },
    {
      ...
      features: [
        ...
        { text: 'Günlük 3 Tarot Kredisi (Yenilenir)', included: true, icon: <Layers className="w-3 h-3 text-blue-400 inline mr-1"/> },
    ...
    },
    {
      ...
      features: [
        ...
        { text: 'Gelişmiş Görsel (Günlük 10 Adet)', included: true, icon: <ImageIcon className="w-3 h-3 text-amber-400 inline mr-1"/> },
  ...
  return (
    <div className="min-h-[100dvh] bg-[#020617] text-white font-sans relative overflow-x-hidden flex flex-col pb-20 md:pb-32">
      ...
      <nav className="sticky top-0 z-30 w-full bg-[#020617]/80 backdrop-blur-md border-b border-white/5 p-4 md:p-6 flex items-center justify-between">
        <button onClick={() => router.back()} className="flex items-center gap-2 p-2 rounded-full bg-white/5 hover:bg-white/10 active:scale-95 transition-all group">
          <ArrowLeft className="w-4 h-4 md:w-5 md:h-5 text-gray-300 group-hover:text-white" /> 
          <span className="text-xs md:text-sm font-bold text-gray-300 group-hover:text-white">Geri Dön</span>
        </button>
        <div className="text-[10px] md:text-xs font-bold text-[#fbbf24] border border-[#fbbf24]/20 px-2 py-1 md:px-3 md:py-1 rounded-full bg-[#fbbf24]/5 flex items-center gap-1.5 md:gap-2">
      ...
      <div className="flex-grow w-full max-w-7xl mx-auto px-4 md:px-6 pt-6 relative z-10">
        <div className="text-center mb-8 md:mb-16">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-3xl md:text-5xl font-serif text-[#fbbf24] mb-3 md:mb-4 tracking-wide">Kaderini Seç</h1>
            <p className="text-gray-400 text-xs md:text-lg max-w-2xl mx-auto px-2 leading-relaxed">
              Bilinçaltının kapılarını ne kadar aralamak istersin? Senin için en uygun rehberlik paketini seç.
            </p>
          </motion.div>
        </div>

        {/* PAKETLER GRID (MOBİLDE TEK KOLON, TABLETTE 2, PCDE 3) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 items-start">
          {plans.map((plan, index) => (
            <motion.div 
              ...
              className={`relative p-6 md:p-8 rounded-2xl md:rounded-3xl border transition-all duration-300 flex flex-col h-full group ${
                plan.popular 
            ...
            >
              {plan.popular && (
                <div className="absolute -top-3 md:-top-4 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-[10px] md:text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-widest shadow-lg flex items-center gap-1 whitespace-nowrap">
              ...
              <div className="mb-6 p-3 md:p-4 rounded-xl md:rounded-2xl bg-black/20 border border-white/5">
                {plan.oldPrice && plan.price !== 'Ücretsiz' && (
                    <span className="text-gray-500 line-through text-[10px] md:text-sm block mb-1">Normal: {plan.oldPrice}</span>
                )}
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl md:text-4xl font-serif text-white tracking-tight">{plan.price}</span>
                  {plan.period && <span className="text-gray-500 text-xs md:text-sm">{plan.period}</span>}
                </div>
              ...
              <ul className="space-y-3 md:space-y-4 mb-6 md:mb-8 flex-1">
                {plan.features.map((feature, i) => (
                  <li key={i} className={`flex items-start gap-2 md:gap-3 text-xs md:text-sm ${feature.included ? 'text-gray-200' : 'text-gray-600'}`}>
                    {feature.included ? (
              ...
              <button
                ...
                className={`w-full py-3 md:py-4 rounded-xl font-bold text-sm md:text-base transition-all flex items-center justify-center gap-2 shadow-lg active:scale-95 ${
                  currentTier === plan.id 
              ...
              >
                ...
                {currentTier !== plan.id && plan.id !== 'free' && <Crown className="w-3 h-3 md:w-4 md:h-4" />}
              </button>
            </motion.div>
          ))}
        </div>
        ...
        <div className="mt-12 md:mt-16 flex flex-col md:flex-row justify-center items-center gap-4 md:gap-6 opacity-50 grayscale hover:grayscale-0 transition-all duration-500 pb-10">
           <div className="flex items-center gap-2 text-[10px] md:text-xs text-gray-400 border border-white/10 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-white/5">
              <ShieldCheck className="w-3 h-3 md:w-4 md:h-4 text-green-500" /> SSL ile Güvenli Ödeme
           </div>
           <div className="flex items-center gap-4">
               <div className="h-4 w-[1px] bg-white/20 hidden md:block"></div>
               <img src="https://upload.wikimedia.org/wikipedia/commons/2/2a/Mastercard-logo.svg" alt="Mastercard" className="h-5 md:h-8" />
               <img src="https://upload.wikimedia.org/wikipedia/commons/5/5e/Visa_Inc._logo.svg" alt="Visa" className="h-3 md:h-5" />
           </div>
        </div>
      </div>
    </div>
